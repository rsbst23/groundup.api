...
public class TenantSsoSettingsRepository : ITenantSsoSettingsRepository
{
    private readonly ApplicationDbContext _dbContext;
    private readonly IMapper _mapper;
    private readonly ILoggingService _logger;

    public TenantSsoSettingsRepository(
    ...
    public async Task<ApiResponse<TenantDetailDto>> ConfigureSsoSettingsAsync(int tenantId, ConfigureSsoSettingsDto dto)
    {
        try
        {
            ...
            _logger.LogInformation(
                $"Updated SSO settings for tenant {tenantId}: Domains={string.Join(",", dto.SsoAutoJoinDomains ?? new List<string>())}, RoleId={dto.SsoAutoJoinRoleId}");
