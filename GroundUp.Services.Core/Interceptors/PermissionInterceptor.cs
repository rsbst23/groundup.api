...
public sealed class PermissionInterceptor : IInterceptor, IPermissionInterceptor
{
    ...
    public void Intercept(IInvocation invocation)
    {
        ...
        MethodInfo? interfaceMethod = interfaceType?.GetMethod(methodName, parameterTypes);
        ...
        if (permissionAttribute.RequiredRoles is { Length: > 0 })
        {
        ...
        if (!isAuthorized && permissionAttribute.Permissions is { Length: > 0 })
        {
            try
            {
